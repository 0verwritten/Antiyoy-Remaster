...
3. **Artifact limit 1 MB**; the client bundle is **~694 KB** after the atlas
   slimming (80px frames, unused sprites stripped). `node
   devtools/check-bundle.mjs` builds and fails above the 900 KB budget — run
...
npx tsx devtools/convert-campaign.ts         # regenerate __generated__/campaign-data.ts from core/
