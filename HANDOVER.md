...
3. **Keep the client bundle small**; the sprite
   ...
   embedded. `render.ts` sets `crossOrigin`
   ...
   to pick up a new file). `node devtools/check-bundle.mjs` reports the
   generated bundle size; pass a byte budget argument when you want a local gate.
...
node devtools/check-bundle.mjs               # build + bundle-size report
